...
func (db *DB) SearchItem(ctx context.Context, searchString string) ([]Item, error) {
	query := `SELECT task, status FROM todo_items WHERE task ILIKE $1`
	rows, err := db.pool.Query(ctx, query, fmt.Sprintf("%%%s%%", searchString))
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (db *DB) GetItem(ctx context.Context, task string) *Item {
	query := `SELECT task, status FROM todo_items WHERE task =$1`

	row := db.pool.QueryRow(ctx, query, task)
	var item Item

	err := row.Scan(&item.Task, &item.Status)
